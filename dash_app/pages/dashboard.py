...
import os, json, random, re, base64, io, uuid, time, socket, calendar, math
...
class UInterface:
	...
	def render_daily_line_chart(self, configs=None):
		print('rendering daily line chart')
		df = self.data['traffic_daily']
		df['plays_per_user'] = df['video_plays'] / df['users']
		item_limit = 5
		if configs == None:
	...
	def render_device_share(self, configs=None):
		print('rendering device share chart')
		df = self.data['traffic_daily']
	...
	def render_category_share(self, configs=None):
		print('rendering video category share chart')
		df = self.data['traffic_daily']
