...
if global_config['enable_google_auth']:
	...
	AUTHORIZED_DOMAINS = []
	try:
		AUTHORIZED_DOMAINS = json.loads(os.environ['AUTHORIZED_DOMAINS'])
	...
	class AuthCredentials:
		def __init__(self, cache_key, encryption_secret_key):
	...
	with server.app_context():
		...
		def ensure_session():
			...
			if 'session_id' not in flask.session:
				...
				server.logger.info(f'New session_id created: {new_session_id}')
